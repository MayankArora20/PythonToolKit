#!/usr/bin/env python3
"""Compare source files against target files, including source subfolders.

This is the extended version of the previous script:
- source files are scanned recursively
- target files are scanned recursively
- path is not a comparison criterion
- it also reports folder structure differences between source and target
...
def is_likely_duplicate(file1: Path, file2: Path, similarity_threshold: float) -> bool:
    ...
    return name_score >= similarity_threshold
...
def build_reason(file1: Path, file2: Path) -> str:
    ...
    ftype = file_type(file1)
    reasons.append(f"type={ftype}")

    size1 = file1.stat().st_size
    size2 = file2.stat().st_size
    reasons.append(f"size_diff={safe_percent_diff(size1, size2):.2%}")
    if size1 == size2:
...
def compare_source_and_target(source_folder: str, target_folder: str, similarity_threshold: float = 0.75) -> Tuple[List[str], List[str]]:
    source_files = collect_files(source_folder, recursive=True)
    target_files = collect_files(target_folder, recursive=True)

    source_dirs = collect_relative_directories(source_folder, recursive=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare source files to target files including source subfolders.")
    ...
    parser.add_argument("--report", help="Optional output file for the report")
    parser.add_argument("--threshold", type=float, default=0.75, help="Name similarity threshold (0.0-1.0). Default: 0.75")
    args = parser.parse_args()

    try:
        report_lines, structure_lines = compare_source_and_target(args.source_folder, args.target_folder, args.threshold)
